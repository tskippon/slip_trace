function [ plane, direction, traceMismatch,slipDirAngle] = slip_trace_BCC(imgfile, ebsdfile,n)
...
CS = { 'notIndexed',crystalSymmetry('m-3m', [3.53 3.53 3.53], 'mineral', 'Zr Beta', 'color', 'light green')};
...
%Define possible slip planes and slip directions for BCC
first=symmetrise(Miller(1,1,0,grain.CS,'hkl'));
second=symmetrise(Miller(1,1,2,grain.CS,'hkl'));
third=symmetrise(Miller(1,2,3,grain.CS,'hkl'));


burgers=symmetrise(Miller(1,1,1,grain.CS,'uvw'),'antipodal');


%Calculate all possible slip trace vectors in crystal coordinates based on
%slip planes and SEM image plane normal
firstTrace=cross(prism,implaneNormal);
secondTrace=cross(basal,implaneNormal);
thirdTrace=cross(pyramidal,implaneNormal);

%Get angle between the user-defined slip trace and the theoretically
%calculated slip trace directions
temp=angle(firstTrace,line,'nosymmetry');
temp2=angle(secondTrace,line,'nosymmetry');
temp3=angle(thirdTrace,line,'nosymmetry');

%Make lists of all possible slip planes and all trace-mismatch angles
allPlanes=[first; second; third];
angles=[temp; temp2; temp3];
